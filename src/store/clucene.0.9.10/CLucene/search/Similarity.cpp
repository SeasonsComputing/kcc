...
CL_NS_DEF(search)
...
#ifdef LUCENE_FALLBACK_BYTE_FLOAT_ENCODING
...
#ifndef LUCENE_FALLBACK_BYTE_FLOAT_ENCODING
	   if ( !NORM_TABLE_initd ){
		for (int i = 0; i < 256; i++)
   ...
   float_t Similarity::tf(int32_t freq) {
  ...
  float_t DefaultSimilarity::lengthNorm(const TCHAR* fieldName, int32_t numTerms) {
    if ( numTerms == 0 ) //prevent div by zero
        return 0; //todo: is this correct? should it get here at all???
  ...
  float_t DefaultSimilarity::queryNorm(float_t sumOfSquaredWeights) {
    if ( sumOfSquaredWeights == 0 ) //prevent div by zero
        return 0.0f; //todo: is this correct? should it get here at all???
  ...
  float_t DefaultSimilarity::coord(int32_t overlap, int32_t maxOverlap) {
    return overlap / (float_t)maxOverlap; //todo: div by zero?
  }
CL_NS_END
