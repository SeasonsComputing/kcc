...
CL_NS_DEF(search)
  ...
  {//MSVC 6 scope fix
    int32_t len = hq->size();
	for (int32_t i = len-1; i >= 0; i--)	  // put docs in array
  ...
  _CLDELETE(hq);
    ...
    return _CLNEW TopDocs(totalHits, scoreDocs);
  }
  ...
  TopFieldDocs* MultiSearcher::_search (Query* query, Filter* filter, const int32_t n, const Sort* sort){
    ...
    for (int32_t i = 0; i < searchablesLen; i++) { // search each searcher
	  ...
	  bool done=false;
      for(int32_t j = 0;j<docs->totalHits;j++){ // merge scoreDocs into hq
		  if ( !done ){
			ScoreDoc* scoreDoc = scoreDocs[j];
			scoreDoc->doc += starts[i];                // convert doc
    ...
    ScoreDoc** scoreDocs = _CL_NEWARRAY(ScoreDoc*,hq->size()+1);
    ...
    _CLDELETE(hq);
    return _CLNEW TopFieldDocs (totalHits, scoreDocs, hqFields);
  }
