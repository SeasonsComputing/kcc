 ...
 * @version $Id: CompoundFile.h 15199 2007-03-09 17:57:17Z tvk $
 */
class CompoundFileReader: public CL_NS(store)::Directory {
	...
	*/
	...
	public:
		CSInputStream(CL_NS(store)::InputStream* base, const int64_t fileOffset, const int64_t length);
		CSInputStream(const CSInputStream& clone);
		~CSInputStream();

		/** Closes the stream to futher operations. */
		void close();
		CL_NS(store)::InputStream* clone();
	};
...
private:
	...
	CL_NS(store)::InputStream* stream;
...
public:
	...
	void close();
	CL_NS(store)::InputStream* openFile(const char* id);
	...
	CL_NS(store)::OutputStream* createFile(const char* name);
...
private:
	...
	*/
	void copyFile(WriterFileEntry* source, CL_NS(store)::OutputStream* os, uint8_t* buffer, int32_t bufferLength);
public:
