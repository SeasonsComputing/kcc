...
private:
  ...
  CL_NS(store)::OutputStream* tvx, *tvd, *tvf;
...
private:
  ...
  CL_NS(store)::InputStream* tvx;
  CL_NS(store)::InputStream* tvd;
  CL_NS(store)::InputStream* tvf;
  int64_t _size;
  
  void checkValidFormat(CL_NS(store)::InputStream* in);
  ...
  int64_t size();
	...
	DEFINE_MUTEX(get1Lock);
	DEFINE_MUTEX(get2Lock);
public:
