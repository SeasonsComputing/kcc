       ...
       while (fields->hasMoreElements()) {
        ...
        if (!field->isTokenized()) { // un-tokenized field
           ...
           if (field->stringValue() == NULL) {
              ...
              const int64_t origReaderPos = r->position();
              try {
                r->seek(0);
                if (r->available() > LUCENE_INT32_MAX_SHOULDBE) {
                  _CLTHROWA(CL_ERR_IO,"CLucene is not equipped to handle field values"
                      " longer than LUCENE_INT32_MAX_SHOULDBE characters."
                    );
                }
                int32_t charsAvailable = (int32_t)r->available();
                TCHAR* charBuf = _CL_NEWARRAY(TCHAR,charsAvailable+1);
                try {
            ...
            if (field->readerValue() != NULL) {
              ...
              reader = _CLNEW CL_NS(util)::StringReader(field->stringValue());
              delReader = true;
            } else {
	...
	void DocumentWriter::writeNorms(Document* doc, const char* segment) {
      ...
      for(int32_t n = 0; n < fieldInfos->size(); n++){
         ...
         if(fi->isIndexed){
            ...
            OutputStream* norms = directory->createFile(fn);
            try {
