...
CompoundFileReader::CSInputStream::CSInputStream(CL_NS(store)::InputStream* base, const int64_t fileOffset, const int64_t length){
   this->base = base;
...
InputStream* CompoundFileReader::CSInputStream::clone()
{
...
InputStream* CompoundFileReader::openFile(const char* id){
  if (stream == NULL)
      _CLTHROWA(CL_ERR_IO,"Stream closed");

	SCOPED_LOCK_MUTEX(openFileLock);
...
OutputStream* CompoundFileReader::createFile(const char* name){
   _CLTHROWA(CL_ERR_UnsupportedOperation,"UnsupportedOperationException");
   return NULL;
}
...
void CompoundFileWriter::copyFile(WriterFileEntry* source, OutputStream* os, uint8_t* buffer, int32_t bufferLength){
  InputStream* is = NULL;
  try {
