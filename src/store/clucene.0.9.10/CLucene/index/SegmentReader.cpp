...
CL_NS_DEF(index)
 ...
 SegmentReader::Norm::Norm(InputStream* instrm, int32_t n, SegmentReader* r, const char* seg): 
  ...
  void SegmentReader::Norm::reWrite(){
      ...
      OutputStream* out = reader->getDirectory()->createFile(buf);
      try {
  ...
  SegmentReader::SegmentReader(SegmentInfos* sis, SegmentInfo* si, bool closeDir) : 
      //Init the superclass IndexReader
      IndexReader(si->getDir(),sis,closeDir),
	  _norms(false,false)
  { 
      initialize(si);
  }
  ...
  void SegmentReader::initialize(SegmentInfo* si){
      ...
      //getDirectory()->__addref(); //because both the indexreader and the segments reader do a directory->close()
                           //we must also do a refInc here.
  ...
  //       otherwise a reference to the found document has been returned
      ...
      SCOPED_LOCK_MUTEX(document_LOCK);
  ...
  //Post - true has been returned if document n has been deleted otherwise fralse
      ...
      SCOPED_LOCK_MUTEX(isDeleted_LOCK);
  ...
  //       You are never sure whether or not the norms have been read into bytes properly!!!!!!!!!!!!!!!!!
   ...
   InputStream* _normStream = norm->in->clone();
    // read from disk
    try{ 
       _normStream->seek(0);
...
/*#ifndef CLUCENE_LITE
  ...
  //Post - The deletions file has been overwritten by the new one
      ...
      char tmpName[CL_MAX_PATH];
      char delName[CL_MAX_PATH];
      //Create a filename for storing the new deletes docs
      reader->SegmentName(tmpName, CL_MAX_PATH,".tmp");
	  //Create a filename for the current deletion file
      reader->SegmentName(delName,CL_MAX_PATH, ".del");
      //Write the deleted docs to the new deletion file
      reader->deletedDocs->write(reader->getDirectory(),  tmpName );
    
	  //Rename the old deletions file into the new one
      reader->getDirectory()->renameFile( tmpName, delName );
      return NULL;
  }
#endif //CLUCENE_LITE
*/
