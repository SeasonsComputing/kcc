...
CL_NS_DEF(index)
	...
	private:
		...
		CL_NS(store)::InputStream* freqStream;
		int32_t skipInterval;
		int32_t numSkips;
		int32_t skipCount;
		CL_NS(store)::InputStream* skipStream;
		int32_t skipDoc;
		int64_t freqPointer;
		int64_t proxPointer;
		int64_t skipPointer;
	...
	private:
		CL_NS(store)::InputStream* proxStream;
		int32_t proxCount;
		int32_t position;
  ...
  /** Called by super.skipTo(). */
	...
	class SegmentReader: public IndexReader{
		...
		private:
			...
			public:
				CL_NS(store)::InputStream* in;
				uint8_t* bytes;
				bool dirty;
				//Constructor
				Norm(CL_NS(store)::InputStream* instrm, int32_t number, SegmentReader* reader, const char* segment);
				//Destructor
				~Norm();
	...
	private:
		...
		DEFINE_MUTEX(norms_mutex);//note: should be locking the actual object, but this is not accessed from anywhere else...
		...
		DEFINE_MUTEX(norms1_LOCK);
		DEFINE_MUTEX(norms2_LOCK);
  ...
  */
		...
		SegmentReader(SegmentInfos* sis, SegmentInfo* si, bool closeDir);
		///Destructor.
      ...
      ///Reads the Norms for field from disk starting at offset in the inputstream
		void norms(const TCHAR* field, uint8_t* bytes, const int32_t offset); //syncronized
